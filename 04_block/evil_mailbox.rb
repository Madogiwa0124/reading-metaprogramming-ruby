...
class EvilMailbox
  def initialize(obj, str=nil)
    @obj = obj
    return unless str
    # 無理やりすぎるので出題者の意図を汲めてなさそう・・・
    def @obj.auth(str); @str = str; super(str); end
    @obj.auth(str)
    def @obj.send_mail(to, body); super(to, body + @str); end
  end

  def send_mail(to, body)
    @obj.send_mail(to, body).then { |bool| yield(bool) if block_given? }
    nil
  end
